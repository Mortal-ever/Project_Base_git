# 15 Coffee2Open 启动、配置、任务与设备绑定

阅读顺序：Config → manager.h → manager.c → Device 初始化 → RTU/Robot 队列。本文适用 Coffee2Open，2026-09-08 当前源码；纳入构建不等于本轮构建通过。
...
`coffee2_app_config.h` 定义网络地址、串口波特率、任务栈、超时、协议选择和 OTA 分区。`coffee2_device_bindings.h` 将私有设备编号绑定到 route/unit/驱动。`coffee2_io_names.h`（以附录实际文件为准）维护产品点位名称；公共 IO 模块不应知道“出餐门”。
...
DeviceBinding 的 `xDeviceId` 用于产品查表；`ucRouteId` 选队列；`ucUnitId` 才进入下行协议。`usMinimumIntervalMs` 控制事务间隔；Category/Role/Driver/Protocol 四字段分别描述类别、作用、型号和线路协议。`pcName` 是静态诊断名。绑定表不能按编号直接假设数组下标，阅读 `pxCoffee2DeviceGetBinding` 的实际查找逻辑。
...
本产品的 `USE_COFFEE2` 选中自己的 manager。Coffee2/Coffee3 暴露相同 `xAppTaskManagerCreateTasks` 符号，只能选择一个产品实现进入一次链接；不要同时纳入两个 manager。
...
SHA-256：`25382EE00196DA978F485717347920DFE5DF5A4C2BBA0C4ADEC452DE3DCED770`。以下行号对应 2026-09-08 工作区快照。
...
SHA-256：`C8F8A9ADAF03B3CC0D1C32D74C1AE9B20D0635836CF73BC255FD584365ED10B2`。以下行号对应 2026-09-08 工作区快照。
...
SHA-256：`7F0090AD3289F94DA9F19D30786C0348F867496112DB51F9785A0A784D07D711`。以下行号对应 2026-09-08 工作区快照。
...
SHA-256：`6E9BBA0950A3EE87BE07F8D4217D6FB00D826EB6D8D3486FF8891E1883E8D494`。以下行号对应 2026-09-08 工作区快照。
...
L27
```c
#define COFFEE2_IO_NAME_MB1_DI_1    "X1_FINISHED_FRONT_CUP"
```


L28
```c
#define COFFEE2_IO_NAME_MB1_DI_2    "X2_FINISHED_REAR_CUP"
```


L29
```c
#define COFFEE2_IO_NAME_MB1_DI_3    "X3_OUTLET_CUP"
```


L30
```c
#define COFFEE2_IO_NAME_MB1_DI_4    "X4_PURE_WATER_BUCKET1_LOW_LEVEL"
```


L31
```c
#define COFFEE2_IO_NAME_MB1_DI_5    "X5_WASTE_RESIDUE_BIN"
```


L32
```c
#define COFFEE2_IO_NAME_MB1_DI_6    "X6_WASTE_LIQUID_BUCKET"
```


L33
```c
#define COFFEE2_IO_NAME_MB1_DI_7    "X7_WASTE_LIQUID_HIGH_LEVEL"
```


L34
```c
#define COFFEE2_IO_NAME_MB1_DI_8    "X8_RESERVED"
```


L35
```c
#define COFFEE2_IO_NAME_MB1_DI_9    "X9_RESERVED"
```
...
L44
```c
#define COFFEE2_IO_NAME_MB2_DO_1    "Y1_WATER_HEATER_RELAY"
```


L45
```c
#define COFFEE2_IO_NAME_MB2_DO_2    "Y2_RESERVED"
```


L46
```c
#define COFFEE2_IO_NAME_MB2_DO_3    "Y3_RESERVED"
```


L47
```c
#define COFFEE2_IO_NAME_MB2_DO_4    "Y4_MILK_BOX_R1_SOLENOID_VALVE"
```


L48
```c
#define COFFEE2_IO_NAME_MB2_DO_5    "Y5_FRUIT_DRINK_A_R2_SOLENOID_VALVE"
```


L49
```c
#define COFFEE2_IO_NAME_MB2_DO_6    "Y6_FRUIT_DRINK_B_R3_SOLENOID_VALVE"
```


L50
```c
#define COFFEE2_IO_NAME_MB2_DO_7    "Y7_RESERVED"
```


L51
```c
#define COFFEE2_IO_NAME_MB2_DO_8    "Y8_RESERVED"
```


L52
```c
#define COFFEE2_IO_NAME_MB2_DO_9    "Y9_RESERVED"
```


L53
```c
#define COFFEE2_IO_NAME_MB2_DO_10   "Y10_FRUIT_DRINK_A_PERISTALTIC_PUMP"
```


L54
```c
#define COFFEE2_IO_NAME_MB2_DO_11   "Y11_FRUIT_DRINK_B_PERISTALTIC_PUMP"
```


L55
```c
#define COFFEE2_IO_NAME_MB2_DO_12   "Y12_ASSIST_PERISTALTIC_PUMP"
```


L56
```c
#define COFFEE2_IO_NAME_MB2_DO_13   "Y13_RESERVED"
```


L57
```c
#define COFFEE2_IO_NAME_MB2_DO_14   "Y14_RESERVED"
```


L58
```c
#define COFFEE2_IO_NAME_MB2_DO_15   "Y15_RESERVED"
```


L59
```c
#define COFFEE2_IO_NAME_MB2_DO_16   "Y16_RESERVED"
```
...
SHA-256：`3B38736E6C5CD0B30E5078B5D66EAC6A9435E123F6774B7C1154742A03D3A68B`。以下行号对应 2026-09-08 工作区快照。

| 定义行 | 函数 | 下行调用（词法检索，非运行时跟踪） |
| --- | --- | --- |
| 115 | `xAppTaskManagerCreateTasks` | lCoffee2LogEarlyWrite, memset, prvCaptureResetCause, prvCreateTaskLogged, prvGetBusLogSource, prvWriteRawStartupFailure, pxCoffee2RtuBusGetConfig, vCoffee2IoInitialize, vCoffee2LogSetTaskReady, vTransportManagerInit, xCoffee2DeviceInitialize, xCoffee2LogInitWithTransport, xCoffee2LogSerialApplyDefault, xCoffee2LogWrite, xCoffee2LogWriteField, xCoffee2RobotTcpInitialize, xCoffee2RtuBusInitialize, xCoffee2SerialApplyDefaults, xCoffee2ServerInitialize, xCoffee2WorkflowInitialize, xEventGroupCreateStatic, xPortGetFreeHeapSize |
| 349 | `vAppTaskManagerRunDefaultTask` | pdMS_TO_TICKS, prvApplyNetworkConfiguration, prvIsNetworkReady, prvPublishNetworkReady, prvUpdateNetworkIndicators, taskENTER_CRITICAL, taskEXIT_CRITICAL, vTaskDelay, xCoffee2LogWrite, xCoffee2LogWriteField, xEventGroupSetBits |
| 378 | `vAppTaskManagerWaitNetworkStackReady` | xEventGroupWaitBits |
| 389 | `ucAppTaskManagerIsNetworkReady` | xEventGroupGetBits |
| 401 | `vAppTaskManagerGetStatus` | taskENTER_CRITICAL, taskEXIT_CRITICAL |
| 412 | `prvPublishNetworkReady` | taskENTER_CRITICAL, taskEXIT_CRITICAL, xCoffee2LogWrite, xEventGroupClearBits, xEventGroupSetBits |
| 438 | `prvIsNetworkReady` | ip_addr_isany, netif_ip_addr4, netif_is_link_up, netif_is_up |
| 450 | `prvApplyNetworkConfiguration` | IP4_ADDR, netif_set_addr |
| 472 | `prvCaptureResetCause` | __HAL_RCC_CLEAR_RESET_FLAGS, __HAL_RCC_GET_FLAG |
| 503 | `prvCreateTaskLogged` | xCoffee2LogWriteField, xTaskCreate |
| 527 | `prvWriteRawStartupFailure` | HAL_UART_Transmit |
| 539 | `prvGetBusLogSource` |  |
| 554 | `prvUpdateNetworkIndicators` | HAL_GPIO_WritePin, pdMS_TO_TICKS, xTaskGetTickCount |
...
SHA-256：`BCD216E3EA19EDFE6EBD536F49B909DFD4E2FA8ABAA031EB5EB78364AF6DAA14`。以下行号对应 2026-09-08 工作区快照。
...
SHA-256：`80496A47FE60B1462BF18BD3994C29E6450E353FD16C64CC308B7DAC3476133B`。以下行号对应 2026-09-08 工作区快照。
...
SHA-256：`8764A0249AD9A3A37EF7AB2A7843942B21E94D7CDC2892B876E107D77209440F`。以下行号对应 2026-09-08 工作区快照。
...
SHA-256：`D7C4825F39531AFED4F5BB581C76650B792751FC47859F56C3AAE3630F93C3BA`。以下行号对应 2026-09-08 工作区快照。

| 定义行 | 函数 | 下行调用（词法检索，非运行时跟踪） |
| --- | --- | --- |
| 53 | `xCoffee2LogInit` | xCoffee2LogInitWithTransport |
| 59 | `xCoffee2LogInitWithTransport` | memset, xAppLogInit, xTransportUartCreate |
| 99 | `xCoffee2LogWrite` | xCoffee2LogWriteOrder |
| 107 | `xCoffee2LogWriteOrder` | xAppLogWriteOrder |
| 117 | `xCoffee2LogWriteField` | xCoffee2LogWriteFieldOrder |
| 127 | `xCoffee2LogWriteFieldOrder` | xAppLogWriteFieldOrder |
| 138 | `xCoffee2LogWriteTextOrder` | xAppLogWriteTextOrder |
| 147 | `xCoffee2LogPrintfOrder` | va_end, va_start, vsnprintf, xCoffee2LogWriteTextOrder |
| 168 | `lCoffee2LogEarlyWrite` | lAppLogEarlyWrite |
| 174 | `vCoffee2LogTask` | vAppLogTask, xCoffee2LogWrite |
| 182 | `vCoffee2LogSetTaskReady` | vAppLogSetTaskReady |
| 188 | `vCoffee2LogGetStatus` | vAppLogGetStatus |
| 194 | `vCoffee2LogLwipResourceFailure` | vAppLwipAlertReportFailure |
### [Application/UserAPP/Coffee2OpenApp/Comm_Log/coffee2_log.h](../../../../Application/UserAPP/Coffee2OpenApp/Comm_Log/coffee2_log.h)

SHA-256：`545A9BB8F823A47556DBFFED90D6E27A5BC6E331FD2E70C7AEB4D3A1B9508525`。以下行号对应 2026-09-08 工作区快照。
...
SHA-256：`AE502B52CCED9B19933FFFA80ADDEE23E08F604A1D431DDCFF85D3DEDE18C140`。以下行号对应 2026-09-08 工作区快照。

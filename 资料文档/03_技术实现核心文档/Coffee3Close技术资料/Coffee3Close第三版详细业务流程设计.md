...
```mermaid
flowchart TD
    A[安全输出 取得有效X1 X2 X3 门与机器人状态] --> B[源序列 夹爪 咖啡机位 压盖位]
    B --> C{还有未检查的源}
    C -- 否 --> Z[必要安全退出 残杯检查完成]
    C -- 是 --> D[依次检查储位1 储位2 出餐口]
    D --> E{找到有效空闲目的地}
    E -- 否 --> F[INIT_FAILED_LOCKED 无位置继续检查]
    E -- 是 --> G[预留目的地 执行该源的取放动作]
    G --> H{动作完成且目的地输入有效}
    H -- 否 --> I[INIT_FAILED_LOCKED 具体动作或通信错误]
    H -- 是 --> J{目的地有杯}
    J -- 是 --> K[WARN 发现残杯 标记占用 非成品]
    J -- 否 --> L[记录本源检查通过 释放空位]
    K --> M[标记本源已执行]
    L --> M
    M --> C
...
```mermaid
flowchart TD
    A[本次X3有杯已确认 机器人退出门区] --> B[DO2降门]
    B --> C{DI2到位}
    C -- 超时或冲突 --> Z[停DO1 DO2 门故障 禁止下一次交付]
    C -- 是 --> D[停DO2 等待本次X3有到无]
    D --> E[无杯开始计时]
    E --> F{连续有效无杯满30秒}
    F -- 重新有杯 --> D
    F -- 数据失效 --> G[取消计时 停止门动作 等有效状态]
    G --> D
    F -- 否且仍无杯 --> E
    F -- 是 --> H[置bit4取餐成功位 保留低位阶段 通常0x100B为0x0015]
    H --> I[DO1升门]
    I --> J{DI1到位}
    J -- 否或超时 --> Z
    J -- 是 --> K[停DO1 物理出餐口已复位]
    K --> L[上位机写0x000B确认后清取餐事件]
    L --> M[无杯且无未确认事件 可再次分配]
```
...
| 项目 | 本版处理/建议 | 状态 |
| --- | --- | --- |
| 取餐成功反馈时点 | 连续无杯30秒确认时发布，升门完成独立记录；若主机必须等门到位需统一协议时点 | PROPOSED |
| 上次取餐ACK与下一交付 | 一个未ACK事件占用反馈槽，禁止覆盖；仍可做有储位的线上单 | PROPOSED |
| 残杯放出餐口后的人工取走 | 不当客户订单，不自动报取餐成功；维护开门如何授权需确认 | UNKNOWN |
| 门放杯姿态、机器人安全退出 | 本版采用门升起关闭时放杯、机器人退出后降门；须验证实际碰撞范围 | UNKNOWN |
| 光栅暂不参与 | 严格按用户要求占位；不能靠杯传感器证明没有手或异物 | CONFIRMED边界/安全缺口 |
| 0x1008完成值 | 外部《咖啡售卖机下位机-通信协议.docx》§4寄存器主表=2，但该DOCX的§5.2“订单完成确认”示例=1（不是本文§5.2）；采用主表2作为方案，主机联调核实 | CONFLICT |
| 0x100B取餐状态值域/组合 | 主表未说明组合且范围列只写0—3；Coffee1源码是低阶段+高成功位，V3按§10.3兼容，常见0x0015 | CONFLICT，已有源码依据 |
| 0x0208位图 | DOCX仍为0x0100—0x8000，用户已要求工程用0x01—0x80；本版采用低8位，不修改DOCX | CONFLICT |
| 门调试地址 | 原协议分别有0x0091升降和0x0092开关门；建议仅0x0092映射本机门，0x0091不支持 | PROPOSED |
| 门页面旧杯托限位 | 0x1091仅填有杯、开/关门到位与光栅显示；杯托升降位保持0；主机不得把不存在的杯托未到位视为门故障 | PROPOSED/主机适配 |
| 果乳/糖浆订单字段 | DOCX同格混有“通道量”与“种类+量”；建议新通道量布局，需与实际主机最终确定后冻结 | CONFLICT |
| 奶路Y4 | 图证实选路用途但不证明全部M50清洗程序均需该阀；程序映射待确认 | UNKNOWN |
| 本机DO实际电平 | 本机DI低有效已在源码确认；DO现有ODR镜像高位为1，与历史“输入输出均低有效”说法需实物裁决 | CONFLICT/UNKNOWN |
| X4水源及液位真值 | 当前配置未定义极性；旧设计初稿“无信号报警”仅作为本版暂拟依据；现场确认前禁止按假设启用自动水路 | UNKNOWN，实施前阻断项 |
| 暂存/废杯占用分类 | 现有协议只有占用位，内部分类和日志可实现；需要主机显示类别须另定协议 | PROPOSED |
| 设备/点位/耗时 | M50菜单、杯盖通道、糖浆位置、机器人动作编号和完成位、各超时待现场资料 | UNKNOWN |
| 打印、紫外灯、多水桶、多出餐口 | 当前BOM无对应配置，非零请求应明确不支持，不静默执行空步骤 | OUT_OF_SCOPE |

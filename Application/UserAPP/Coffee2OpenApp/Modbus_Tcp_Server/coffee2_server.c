...
{
	if (usAddress == COFFEE2_REG_LOCAL_IO_DEBUG) {
		if (ucCoffee2WorkflowInitializationComplete() == 0U) {
		...
		(void)ucCoffee2IoApplyLocalDebugMask(usValue);
	...
	uint16_t usParameter1)
{
	...
	ucDebug = ((xAction == COFFEE2_ACTION_IO_WRITE_MASK) ||
		((xAction >= COFFEE2_ACTION_ROBOT_START) &&
		 (xAction <= COFFEE2_ACTION_ROBOT_MANUAL_MODE))) ? 1U : 0U;
	if ((ucDebug != 0U) &&
		(ucCoffee2WorkflowInitializationComplete() == 0U)) {
		(void)xCoffee2LogWriteFieldOrder(COFFEE2_LOG_LEVEL_WARNING,
			COFFEE2_LOG_SOURCE_SERVER, COFFEE2_LOG_ORDER_DEBUG,
	...
	if ((xAction == COFFEE2_ACTION_IO_WRITE_MASK) ||
		((xAction >= COFFEE2_ACTION_ROBOT_START) &&
		 (xAction <= COFFEE2_ACTION_ROBOT_MANUAL_MODE))) {
		xCommand.ucFlags |= COFFEE2_COMMAND_FLAG_DEBUG;
	}
	if (xAction == COFFEE2_ACTION_IO_WRITE_MASK) {

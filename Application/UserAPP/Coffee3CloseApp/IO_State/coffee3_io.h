...
typedef struct {
	uint8_t aucXPin[COFFEE3_LOCAL_IO_COUNT]; /*!< Eight local DI logical values after active-low normalization. */
	uint8_t aucMB1XPin[COFFEE3_MODBUS_IO_COUNT]; /*!< Sixteen input-module values read from Unit 1. */
	uint8_t aucMB2XPin[COFFEE3_MODBUS_IO_COUNT]; /*!< Reserved second input array; not a second installed input module. */
} Coffee3InputIo_t;

/** @brief Store all output points consumed by workflow and device tasks. */
typedef struct {
	uint8_t aucYPin[COFFEE3_LOCAL_IO_COUNT]; /*!< Eight local output levels; not relay contact feedback. */
	uint8_t aucMB1YPin[COFFEE3_MODBUS_IO_COUNT]; /*!< Reserved first module output array in the internal legacy layout. */
	uint8_t aucMB2YPin[COFFEE3_MODBUS_IO_COUNT]; /*!< Sixteen Unit 2 outputs; projected to host external output group 1. */
} Coffee3OutputIo_t;

/** @brief Store one coherent global IO image and update metadata. */
typedef struct {
	Coffee3InputIo_t xInput; /*!< Local and module input image owned by this product. */
	Coffee3OutputIo_t xOutput; /*!< Local and module output image owned by this product. */

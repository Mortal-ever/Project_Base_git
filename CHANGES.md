...
| --- | --- | --- | --- |
| 2026-09-22 | Application/UserAPP/Coffee3CloseApp/Modbus_Rtu_Bus/coffee3_rtu_bus.c | Modify | Keep each RTU background transaction intact through response or timeout; remove foreground preemption and preserve queue priority for the next transaction. |
| 2026-09-22 | Application/UserAPP/Coffee3CloseApp/Device/coffee3_device.c | Modify | Stop requesting RTU read cancellation when foreground commands are queued. |
| 2026-09-22 | Application/UserAPP/Coffee3CloseApp/Config/coffee3_app_config.h | Modify | Change ordinary RTU health polling period from 3 seconds to 5 seconds; retain the 100 ms IO cadence. |
| 2026-09-22 | Application/UserAPP/Coffee3CloseApp/Modbus_Rtu_Bus/coffee3_rtu_bus.h | Modify | Remove the obsolete Coffee3 RTU preemption API from the public bus interface. |
...
| 2026-09-04 | `资料文档/03_技术实现核心文档/Coffee3Close技术资料/Coffee3Close第五版详细业务流程设计.md` | Add | 按第四版审核建议输出第五版：修正X4制作中缺水为“本单正常完成、完成后禁止新单”，补充Coffee1单一出餐反馈槽处理、T06资源预留细节、热杯1/冷杯2默认位置、糖浆Coffee1兼容与A/B果乳、中英双语IO表，以及强制source前缀但正文可读的日志规则。 |
| 2026-09-04 | `资料文档/00_README/工程基础缓存.md`; `资料文档/全局审查.md` | Update | 记录Coffee3Close V5文档候选、审核结论和未决协议同步项；本轮未修改源码、协议DOCX、Keil/GCC、IOC，未构建或烧录。 |
| 2026-09-07 | `资料文档/03_技术实现核心文档/06_日志/日志输出标准与约束.md` | Add | 固化当前 STM32 日志的稳定 `[ORDER_HEX4LEVEL][TASK:MODULE]` 前缀、source 与事件正文边界、级别、去重、IO/订单/设备日志最小信息集、Coffee3 接入清单和验收项；明确 `C3Outlet/C3Water/C3IoInput` 不能作为未注册动态 source。 |
| 2026-09-07 | `资料文档/03_技术实现核心文档/06_日志/日志系统总体实现说明.md` | Update | 增加实现说明与输出标准的边界说明，引用当前 `app_log.c` 的格式化路径及 Text/Field API 选择规则。 |
| 2026-09-07 | `资料文档/03_技术实现核心文档/Coffee3Close技术资料/Coffee3Close第五版详细业务流程设计.md` | Correct | 修正第五版日志示例：事件对象不再伪装成 `C3Outlet/C3Water/C3IoInput` source，改用稳定的 Workflow/Robot/Bus/Server owner 前缀，事件细节放入正文。 |
| 2026-09-07 | `资料文档/00_README/工程基础缓存.md`; `资料文档/全局审查.md` | Update | 登记当前日志输出标准、源码证据和 Coffee3 V5 示例修正；本轮未修改固件、Keil/GCC、IOC、协议 DOCX，未构建或烧录。 |
| 2026-09-07 | `资料文档/03_技术实现核心文档/04_执行报告与审计/Coffee2工程锐评与Coffee3落地建议.md` | Add | 对照 Coffee1 已落地业务和 Coffee2/Coffee3 需求，审查公共/私有边界、业务闭环、设备后置条件、出餐状态、异常恢复、协议冲突、双工具链和 Coffee3 落地形态；给出 P0/P1/P2 风险及分阶段最小改造方案。 |
| 2026-09-07 | `资料文档/03_技术实现核心文档/Coffee3Close技术资料/Coffee3Close第六版详细业务流程设计.md` | Add | 根据审批版和 V5 形成 V6：残杯检查仅使用两个存储位，第三杯无位置则初始化失败并重启；线上订单增加下位机储位二次拦截，满位直接 `0x1008=3`；线下订单不因存储位满被拦截；固化 FC03/FC06/FC10、公共/私有边界、任务清单、流程图和验收矩阵。 |
| 2026-09-07 | `C:/Users/13193/.codex/config.toml`; `C:/Users/13193/.codex/AGENTS.md`; `C:/Users/13193/.codex/agents/sol-high-specialist.toml`; `C:/Users/13193/.codex/skills/embedded-dev/SKILL.md`; `C:/Users/13193/.codex/skills/stm32-keil-v5/SKILL.md`; `C:/Users/13193/.codex/skills/stm32-keil-v5/references/style-and-naming.md`; `C:/Users/13193/.codex/skills/stm32-keil-v5/references/project-safety.md`; `.agents/工程维护Agent提示词.md` | Optimize | 将 Codex 根模型路由适配 GPT-6 Astra，默认推理调整为 medium，委派改为按收益触发，Sol specialist 改为兼容性 fallback，精简嵌入式技能和项目维护提示词，清理固定称呼/话术，移除废弃 IOC 读取要求及当前配置中的密钥样式注释。 |
| 2026-09-07 | `资料文档/03_技术实现核心文档/04_执行报告与审计/Codex_Astra技能与子代理优化报告.md`; `资料文档/00_README/工程基础缓存.md`; `资料文档/全局审查.md`; `CHANGES.md` | Add/Update | 记录 Astra 技能与子代理优化内容、修改后路由、TOML/Skill 验证、文件哈希和生效限制；未修改固件、Keil/GCC、IOC，未构建或烧录。 |
| 2026-09-07 | `Application/UserAPP/Coffee2App/WorkFlow/coffee2_workflow.c/.h`; `Modbus_Tcp_Server/coffee2_server.c/.h`（同 Coffee2App） | Fix | 按 Coffee2 审批修正出餐状态/订单回显所有权、ACK 门控、动作前及放杯前空位复查；增加静态手动预留、维护互斥、事务诊断与恢复锁，保留原始失败原因；不引入 Coffee3 储位/门/额外握手。 |
| 2026-09-07 | `.vscode/c_cpp_properties.json`; `.vscode/launch.json`; `.vscode/tasks.json`; `GCC-ARM/scripts/build.ps1`; `GCC-ARM/scripts/flash.ps1` | Fix | GCC 工具链 Target 对齐：移除 MilkTea（OUT_OF_SCOPE），Coffee2Open 与 Coffee3Close 平级。c_cpp 智能感知与 launch 调试改为两个真实 preset（Coffee2Open/Coffee3Close Debug）；tasks 重建为两产品各 Build/Fresh/Release/Program 共 8 任务；build.ps1 与 flash.ps1 的 ValidateSet 从 `Coffee2/MilkTea` 更新为 `Coffee2Open/Coffee3Close`（默认 Coffee2Open）；仅改配置，未构建、未烧录。 |
| 2026-09-07 | `Application/UserAPP/Coffee2App/Device/coffee2_device.c/.h`; `Modbus_Rtu_Bus/coffee2_rtu_bus.c`（同 Coffee2App） | Fix | 取消按 epoch 前检，终态按 commandId+epoch 对应，安全停止只认实际成功；IO 调试单请求单掩码入队并读回验证，协议错误时禁止提交未读取的零图像。 |
| 2026-09-07 | `Application/DeviceLibrary/CoffeeMachine/coffee_machine_f200.c/.h`; `Application/DeviceLibrary/SyrupMachine/CurrentModbus/syrup_machine_modbus.c/.h` | Fix | 落实现有可选取消回调的发送前/多事务间检查，NULL 保持兼容；公共层不引用 Coffee2，不编造糖浆 STOP 协议。 |
| 2026-09-07 | `Application/UserAPP/Coffee2App/Config/coffee2_device_bindings.h`; `MDK-ARM/STM32F407_Base.uvprojx` | Add/Update | 集中 Coffee2 静态设备绑定和诊断名，沿用原10设备选型；Keil 原私有组加入配置头文件，无新 target/翻译单元，GCC 原 include 可解析。 |
| 2026-09-07 | `tests/test_coffee2_correction_contracts.py`; `GCC-ARM/scripts/build_snapshot.py` | Add | 20 项静态源码契约检查；增加仅消费既有 Coffee2-Debug 图的全量 GCC 诊断构建脚本，绕过当前 Ninja 实际调度挂起，生成可核对的新产物及哈希 manifest；不配置、不清理、不烧录。 |
| 2026-09-07 | `资料文档/03_技术实现核心文档/04_执行报告与审计/Coffee2修正实施与上板验收报告.md`; `资料文档/00_README/工程基础缓存.md`; `资料文档/全局审查.md`; `CHANGES.md` | Add/Update | 记录实施范围、协议事实、异常恢复、源码入口、20项静态测试、Keil零错误零警告及GCC166翻译单元全量通过、map/固件哈希与上板清单；普通Ninja入口仍异常，硬件待验收；Coffee3/IOC/Bootloader未改动。 |
| 2026-09-07 | `MDK-ARM/Objects/Coffee2/*`; `MDK-ARM/Objects/Coffee2_Lst/Coffee2.map`; `GCC-ARM/build/Coffee2-Debug/*`; `GCC-ARM/build/Coffee2-Debug-diag/*`; `MDK-ARM/.vscode/keil-assistant.log` | Generated | 构建/诊断工具生成日志、对象、固件与临时配置；保留失败和成功证据，不作为手工源码修改，不提交或删除用户既有产物。 |
| 2026-09-07 | `Application/UserAPP/Coffee3CloseApp/`; `Application/DeviceLibrary/CoffeeMachine/coffee_machine_m50.c/.h`; `Application/CMakeLists.txt`; `GCC-ARM/CMakeLists.txt`; `GCC-ARM/CMakePresets.json`; `MDK-ARM/STM32F407_Base.uvprojx`; `MDK-ARM/ScatterFiles/Coffee3Close_CCM.sct` | Add/Modify | 创建 Coffee3CloseV3.0.0 独立 target；接入 M50 公共设备库、双储位/单出餐口 Workflow、IO/OTA 仲裁和 GCC/Keil 构建图；不改 IOC、不烧录。 |
| 2026-09-07 | `Application/UserAPP/Coffee3CloseApp/WorkFlow/coffee3_workflow.c/.h`; `Modbus_Tcp_Server/coffee3_server.c`; `IO_State/coffee3_io.c/.h`; `DeviceLibrary/CoffeeMachine/coffee_machine_m50.c`; `tests/test_coffee2_correction_contracts.py` | Fix/Test | 增加门互锁、M50 水箱补水保护、周期 IO refresh 抑制、OTA 空闲输出校验、M50 单事务超时和 Coffee3 静态契约；Keil 0/0，静态测试 22 项通过。 |
| 2026-09-07 | `资料文档/03_技术实现核心文档/04_执行报告与审计/Coffee3CloseV3.0.0创建与验证报告.md`; `资料文档/00_README/工程基础缓存.md`; `资料文档/全局审查.md` | Add/Update | 记录 Coffee3 target 边界、构建证据、GCC 当前源码验证阻塞和上板验收清单。 |
| 2026-09-07 | `MDK-ARM/STM32F407_Base.uvprojx`; `GCC-ARM/CMakeLists.txt`; `GCC-ARM/CMakePresets.json`; `GCC-ARM/scripts/build.ps1`; `Application/CMakeLists.txt`; `.vscode/tasks.json`; `.vscode/launch.json`; `.vscode/c_cpp_properties.json` | Rename | 统一产品 Target 名称为 `Coffee2Open`、`Coffee3Close`；版本号保留在固件版本日志，不再出现在 Target 名称和输出文件名。 |
| 2026-09-07 | `Application/UserAPP/Coffee2App/Device/coffee2_device.c`; `Application/UserAPP/Coffee3CloseApp/Device/coffee3_device.c`; 两 Target `Robot_Tcp/coffee{2,3}_robot_tcp.c` | Fix | Server 手动非 REFRESH 动作优先队首入队；等待受理状态日志标识改为 0000；不修改事务身份、RTOS 优先级或安全准入。十进制520/521地址不变。 |
| 2026-09-07 | `tests/test_manual_priority_contracts.py`; `资料文档/03_技术实现核心文档/04_执行报告与审计/双Target手动优先级与等待日志修正.md`; `资料文档/00_README/工程基础缓存.md`; `资料文档/全局审查.md`; `CHANGES.md` | Add/Update | 两Target Keil均0错误0警告，24项静态契约通过；记录优先级/日志与互锁边界，本轮未GCC构建、未烧录。生成日志为两Target Objects目录的manual_priority.log。 |
## 2026-09-07

- 补充 Coffee3 RTU 失败日志上下文：增加结果名称、Bus、Unit、设备、动作、来源和步骤字段；`RESULT=-4` 明确对应 `MODBUS_PORT_RESULT_TIMEOUT`。
- 明确机器人模拟 Server 未清除命令线圈时保持 `ROBOT_ACTION_ACCEPT_WAITING` 属于预期等待态，不作为设备故障误报。
- 调试输出命令不再被自动流程/维护/OTA 所有权门禁拒绝；更新对应契约测试。旧固件仍可能打印 `IO debug rejected...`，需重新构建并下载后验证。
| 2026-09-07 | Coffee2/Coffee3 Server、IO_State、Workflow；调试指令一次性执行与初始化门禁修正.md | Modify/Add | 调试 IO/机器人命令取消输出所有权互斥，板载 mask 支持一次性 0x000F 等组合写入；仅保留初始化完成门禁。双 Target Keil 0错误0警告。 |
| 2026-09-08 | `资料文档/03_技术实现核心文档/02_源码解读/源码详解/*`；`资料文档/03_技术实现核心文档/04_执行报告与审计/当前工程完整源码带读文档重建方案.md` | Add | 建立编号00-22共23份源码带读教材、源码覆盖矩阵和符号总索引；公共、Coffee2Open、Coffee3Close分别讲解，未修改固件或构建配置。 |
...
| 2026-09-17 | Application/UserAPP/Coffee3CloseApp/Config/coffee3_config.c/.h; Config/coffee3_app_config.h; Modbus_Tcp_Server/coffee3_server.c; WorkFlow/coffee3_workflow.c; tests/test_coffee3_config_contracts.py | Add | Coffee3 以 0x007E 直接配置落冰斜率（ms/g，0 恢复默认 20），沿用唯一 Coffee3Config_t 在 Flash 持久化并兼容旧配置；改参时复用工作流手动占用防止开阀期间擦写 Flash，单次落冰周期锁定斜率，读回有效值并补充日志与契约测试。原有 2000 ms 单脉冲上限和补冰容差未改，需实测校准。 |
| 2026-09-17 | Application/UserAPP/Coffee3CloseApp/Config/coffee3_app_config.h; Config/coffee3_config.h; Modbus_Tcp_Server/coffee3_server.h; WorkFlow/coffee3_workflow.c; tests/test_coffee3_config_contracts.py | Change | 以 Coffee1 Modbus 0x000A 阀控分支的 0.8/1.2 s 首轮、10 ms/g + 100 ms 补冰、200 ms 最短脉冲、1.6 s 落冰后等待和最多三次补冰为 Coffee3 初始模型；0x007E 继续直接表示可持久化 ms/g 斜率，未配置默认改为 10。独立保持 2 g 去皮容差，出冰完成改为目标 ±15 g；超量 WARN 后完成，三次补冰后仍不足则失败。保留 2 s 单次上限，需上板重新校准。 |
| 2026-09-18 | Application/DeviceLibrary/IceMachine/CurrentModbus/ice_machine_modbus.c/.h; tests/test_ice_machine_nonstandard_fc03.py | Fix | 协侯制冰机 FC03 响应以两字节起始地址回显替代标准 Byte Count；仅在公共设备库驱动中使用现有 raw PDU 接口读取、核对回显并解析 13 个寄存器，保留公共 RTU CRC 校验。增加现场 10/13 寄存器抓包向量及驱动路由契约测试。 |
| 2026-09-18 | Application/UserAPP/Coffee3CloseApp/WorkFlow/coffee3_workflow.c/.h; Modbus_Tcp_Server/coffee3_server.c; Modbus_Rtu_Bus/coffee3_rtu_bus.c; tests/test_coffee3_manual_ice_contracts.py | Fix | Coffee3 手动落冰寄存器 0x0071 改按 Coffee1 量纲解释为克，进入现有称重闭环前换算为 0.1 克并检查 16 位范围；拒绝时输出具体准入原因与设备就绪掩码，只有接受后才打印空杯假定及接受日志。修复 RTU 设备刷新成功只发布 online、未发布 ready 导致制冰机与电子秤始终无法通过准入的问题；通信失败仍沿用失效处理，并增加契约测试。 |
| 2026-09-18 | Application/DeviceLibrary/Scale/BSQ_DG_V2/scale_bsq_dg_v2.c/.h; Application/UserAPP/Coffee3CloseApp/Modbus_Rtu_Bus/coffee3_rtu_bus.c; WorkFlow/coffee3_workflow.c | Fix | Coffee3 对现场 BSQ-DG-V2 原始显示值按克归一化为 0.1 克，避免设备上报 unit=2 时被错误放大 1000 倍；保留公共驱动原入口供其他 target 按协议单位换算，并在落冰日志中同时输出 normalized_dg、raw、decimals、reported_unit。重量不足继续进入最多三次补冰，超重仅 WARN 并按成功完成。Coffee2Open/Coffee3Close GCC Debug、Keil ARMCC V5.06u7 均构建通过，相关 16 项契约测试通过。 |
| 2026-09-18 | Application/DeviceLibrary/Scale/BSQ_DG_V2/scale_bsq_dg_v2.c/.h; Application/UserAPP/Coffee3CloseApp/Config/coffee3_app_config.h; WorkFlow/coffee3_workflow.c/.h; tests/test_coffee3_manual_ice_contracts.py; tests/test_coffee3_config_contracts.py | Fix | 复核 Coffee1 量产订单与 0x0071 单步制冰路径后，将 Coffee3 制冰内部标准从 0.1 g 统一为整数 g：订单和调试目标不再乘 10，称重使用 lWeightGram，120 g 初始脉冲分界、2 g 去皮容差、±15 g 完成容差、补冰偏差及日志均按 g 计算；按现场排查要求保留 ICE_WEIGHT_SAMPLE 的 normalized_dg 键名，但其数值为整数克。公共驱动继续保留 lWeightTenthGram 供其他 target 使用。Coffee2Open/Coffee3Close GCC Debug、Keil ARMCC V5.06u7 均 0 warning/0 error，17 项相关契约测试通过。 |

| 2026-09-20 | Application/UserAPP/Coffee3CloseApp/Modbus_Rtu_Bus/coffee3_rtu_bus.c/.h; Application/UserAPP/Coffee3CloseApp/WorkFlow/coffee3_workflow.c; Application/ProtocolStack/ModbusPort; Application/Transport | Add | Coffee3 RTU Bus 接管设备健康轮询：普通设备约 3 秒独立轮询、连续 2 次实际健康帧失败才离线，Bus5 外部 IO 以 100 ms 错相轮询、连续 3 次才离线；前台命令可放弃后台读帧并优先执行，随后从被放弃设备重新发送查询。在线/离线不再由墙钟时间或前台命令失败直接判定，Workflow 停止向 RTU Bus 周期投递刷新命令。制冰流程日志补充 TARE、稳定等待、开关阀、称重、补偿和完成原因，重量日志使用 g 与 last 字段。Keil ARMCC V5.06u7 Coffee3Close 构建 0 warning/0 error。 |
| 2026-09-20 | Application/UserAPP/Coffee3CloseApp/Modbus_Rtu_Bus/coffee3_rtu_bus.c | Change | RTU 日志移除正文中重复的 bus/device 字段，利用日志来源前缀提供总线和设备上下文；Modbus 从站 `unit` 统一改为更直观的 `id`，保留结果码和设备动作码用于深入排查。 |
| 2026-09-20 | Application/UserAPP/Coffee3CloseApp/Modbus_Rtu_Bus/coffee3_rtu_bus.c; Application/UserAPP/Coffee3CloseApp/Device/coffee3_device.c | Fix | 修正 RTU 健康轮询：制冰机特殊 FC03 回显帧中的设备故障不再误判为通信协议失败；有效响应保持 online，故障仅影响 ready。健康失败日志按首次失败和阈值确认去重，后台维护帧不再重复输出通用动作失败日志。 |
| 2026-09-20 | Application/DeviceLibrary/IceMachine/CurrentModbus/ice_machine_modbus.c/.h; Application/UserAPP/Coffee3CloseApp/Modbus_Rtu_Bus/coffee3_rtu_bus.c; Modbus_Tcp_Server/coffee3_server.c | Fix | 制冰机进水、水位和电机过载报警改为可组合的可读日志，并仅在故障掩码变化时输出；上位机 4208 在在线且存在设备报警时返回 0x0003，4209 至 4222 按 Coffee1 状态页语义完整刷新，不支持的状态明确写 0，避免残留旧值。 |
| 2026-09-20 | Application/UserAPP/Coffee3CloseApp/Config/coffee3_app_config.h; WorkFlow/coffee3_workflow.c/.h; Modbus_Tcp_Server/coffee3_server.c; tests/test_coffee3_manual_ice_contracts.py | Fix | Coffee3 手动出冰取消“假设有杯”：参考 Coffee1 量产流程，在去皮和开阀前等待称重稳定，以大于 6 g、最大 100 g 判断杯子，连续 3 次未检出才报无杯，称重通信最多重试 10 次。判杯失败不进入出冰，也不触发设备中止或复位锁定；日志明确给出重量、重试次数和原因。 |
| 2026-09-20 | Application/UserAPP/Coffee3CloseApp/Config/coffee3_app_config.h; WorkFlow/coffee3_workflow.c; tests/test_coffee3_manual_ice_contracts.py; tests/test_coffee3_config_contracts.py; 资料文档/01_项目资料/制冰机协议/Coffee3制冰出冰安全范围与订单失败流程.md | Fix | Coffee3 在残杯初始化完成后清除历史皮重、执行临时清零并建立可迟上线重试的空秤基准；订单在机器人放杯前采集当次空秤、放杯后以差值判杯，手动出冰以上电空秤基准判杯。出冰阶段取消杯上硬件去皮，统一按“当前总重－杯上起始总重”计算冰重，避免拿杯后负皮重影响下一次调试；称重掉线会使基准失效并在安全空闲边界重建。Coffee3Close GCC Debug 与 Keil ARMCC V5.06u7 构建通过，68 项测试通过。 |
| 2026-09-21 | Application/UserAPP/Coffee3CloseApp/Robot_Tcp/coffee3_robot_tcp.c; WorkFlow/coffee3_workflow.c/.h; Device/coffee3_device.c; Modbus_Tcp_Server/coffee3_server.c; tests/test_coffee3_robot_execution.py | Change | Coffee3 机器人订单动作的接取、运动和重连对账超时改为单次 overdue 告警并继续等待完成线圈，订单仅由明确取消终止；调试动作使用单个最新值槽，订单动作期间缓存且不抢占，订单释放机器人后优先执行，活动调试动作可被更新的调试动作立即替换。相关日志统一携带可读 device 名称，普通日志不重复输出 bus/unit。GCC Coffee3Close Debug、Keil ARMCC V5.06u7 和 68 项测试通过。 |
| 2026-09-21 | Application/DeviceLibrary/CoffeeMachine/coffee_machine_m50.c/.h; Application/UserAPP/Coffee3CloseApp/Device/coffee3_device.c; Modbus_Rtu_Bus/coffee3_rtu_bus.c; Modbus_Tcp_Server/coffee3_server.c; WorkFlow/coffee3_workflow.c/.h; tests/test_coffee3_config_contracts.py | Fix | M50 单次调试与订单制作统一使用 180000 ms 超时；前台制作在唯一一次 FC06 后自主 FC03 轮询状态，连续 3 次瞬态读失败才终止并实时发布 working/idle。封闭式取餐按 Coffee1 语义改为 0x0007=0、0x0009=储位1/2、0x000A=出餐口1及物理杯信号共同准入，RAM 订单号仅作日志关联，重启后不再误拒实体杯；机器人动作前再次检查订单门禁。新增两项持久化静态契约测试并通过，未烧录。 |
| 2026-09-23 | Application/UserAPP/Coffee3CloseApp/WorkFlow/coffee3_workflow.c; tests/test_coffee3_robot_execution.py; 资料文档/03_技术实现核心文档/04_执行报告与审计/Coffee3Close机器人自动回HOME与冗余HOME动作调研报告.md; MDK-ARM/BuildLogs/Coffee3Close_home_cleanup_20260923.log | Fix | 按 Coffee1 已落地流程和 Coffee3 机器人动作语义，删除正常订单开始 step 30、订单最终放置后 step 190 及独立取餐放出餐口后 step 820 的冗余 HOME。PUT_STORAGE/PUT_OUTPUT 的完成位作为动作终点，保留人工 HOME 调试入口，并增加防回归契约测试。Coffee3Close GCC Debug 与 Keil ARMCC V5.06u7 构建通过，Keil 0 error/0 warning，85 项契约测试通过，未烧录。 |
| 2026-09-24 | Application/Common（17 个 C/H）；Application/UserAPP/Coffee3CloseApp（除已审核 WorkFlow 外的 27 个 C/H） | Docs | 按用户批准的中文注释标准为所有实际函数定义、变量及类型成员补全注释，并在长函数内说明真实步骤；声明处不重复函数头，修正 OTA、设备镜像、RTU 轮询、Server 异步请求与机器人重连等旧注释。仅修改注释，44 个文件的非注释内容与任务开始快照一致；Coffee2Open/Coffee3Close 受影响的 25 条 GCC 单文件编译命令、git diff --check、UTF-8 与末尾换行检查通过。未执行 Keil 完整构建或硬件验证。 |
